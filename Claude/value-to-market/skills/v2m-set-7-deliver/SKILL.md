...
## On invocation
Read `${CLAUDE_PLUGIN_ROOT}/references/output-protocol.md`, `${CLAUDE_PLUGIN_ROOT}/references/report-format.md`, `${CLAUDE_PLUGIN_ROOT}/references/customer-fit.md`, `${CLAUDE_PLUGIN_ROOT}/references/cheatsheets.md`. List the reasoning files and the existing deliverables, note which sources are newer than their built artifacts, and confirm with the user which deliverables to refresh or build (defaulting to only what changed; the full report only if asked).
